    HIP: 6
    Title: HLC-compatible-Across-Chain
...
# Abstract
HLC Across the chain technology is mainly to solve different main chain (such as: ` HLC ` and ` BTC `) problem of data exchange between the most direct cross chain technology application is "DEX ：distributed exchange", No third party participation will purse ` BTC ` into ` HLC `.
